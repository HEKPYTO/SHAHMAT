...
# mismatch aborts with NO artifact (report moves into place only on success).
...
# (median, gate, JSON emit) fits any service with stable
# "<label>: <value>" stdout markers — adapt the entrypoint/verb/markers
# (here: /svc, perft, nodes/time/nps) to yours.
set -eu
cd "$(dirname "$0")/.."
...
run() { # <name> <fen> <depth> <expected-nodes>
  ...
  [ -n "$nps" ] && [ -n "$secs" ] || { echo "FAIL $1: empty nps/time parse" >&2; exit 1; }
