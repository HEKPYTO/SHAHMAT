#!/bin/sh
# searchbench.sh — search-representative perft matrix (mid/endgame-heavy).
# Every cell (entry x mode) is node-gated: a count mismatch aborts with NO
# artifact. --gate checks exactness only; default also times (warmup + median
# of 5) and writes a JSON report.
# Usage: ./bench/searchbench.sh [--gate] [binary]  (default ./target/release/shahmat-svc)
#
...
OUT="outputs/searchbench-${TS}.json"
TMP="$OUT.tmp"
trap 'rm -f "$TMP"' EXIT
mkdir -p outputs
TBL=""
