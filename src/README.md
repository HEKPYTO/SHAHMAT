...
| `search` | Lib-scoped search substrate (K0) | Fixed-depth alpha-beta + `search_iterative` (no clock, deterministic rows), bound-flag `SearchTt` (exact/lower/upper, mate ply-adjust, probe/store counters; perft table untouched), `evaluate`/`move_score`/`order_moves`, `MATE`; CLI `search` subcommand. No quiescence/pruning/reductions (engine tickets, out of scope) |
