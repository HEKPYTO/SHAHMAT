...
| `search` | Lib-scoped search substrate (K0+K1) | Fixed-depth alpha-beta + `search_iterative` (no clock, deterministic rows), bound-flag `SearchTt` (exact/lower/upper, mate ply-adjust, probe/store counters; perft table untouched), `evaluate`/`move_score`/`order_moves`, `MATE`; K1 quiescence at depth-0 (stand-pat STM eval, captures+promos MVV-LVA, in-check evasion, `MAX_QPLY` 8, table-free; SEE-prune deferred to K4); CLI `search` subcommand. No pruning/reductions (engine tickets, out of scope) |
